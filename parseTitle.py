...
collaborat|reliable|Multi hop|multi turn|Multi step|diagnosis|decision making|temporal abstraction|scalab|asynchron|rare|in the wild|\
...
multilingual|moderation|budget|constrain|salien|interven|llm as a judge|out of distribution|outlier|iid|horizon|panoptic segmentation|\
diagram analysis"
...
pedestrian detection|open dataset|open set|knowledge distillat|style transfer|character recogni|character region detect|text spot|facial expression|\
...
action recognition|re identification|image classifi|deformab matching|deformable object|non rigid|weak supervision|transformer|auto\s*encoder|decoder|\
...
Curriculum|certification|equivarian|(?:normaliz(?:ing|ed) flow)|world model|flow network|multi agent|multi session|long context|feature selection|model selection|data selection|subset selection|\
latent|quantum|low rank|instruction|submodular|vision and language|vision lanugage|visual language|speech language|\
...
information theoretic|chain of thought|codebook|word2vec|covarian|dictionary|online|offline|policy optimization|value based|off policy|on policy|policy gradient|homogeneous|Heterogeneous"
...
articulated|lasso|ridge|mixture of expert|foundation model|mamba|generative model|Markov decision process|Markov chain Monte carlo|partial(?:ly) observab|Random forest|\
Actor critic|latent diffusion|predictive control|Bayesian flow|flow matching|Gaussian splat|gaussian process|adam|singular"
        self.malist = "OOD|RL|L\s*1|L\s*0|CAD|3D|DNA|RNA|EEG|EMG|EM|ODE|BRDF|SLAM|CNN|R CNN|RCNN|DNN|SNN|RNN|LSTM|CTC|SOM|GAN|ICA|PCA|CRF|MRF|MCMC|LDA|U NET|UNET|Nerf|SVM|SVD|eigen|Bagging|MOE|ODE|PDE|LLM|SLM|LVLM|VLM|MM LLM|E2E|\
